...
Este documento es la fuente de verdad de **qué se ha hecho, qué se está haciendo y qué falta**. La idea es que tanto ustedes como una IA (analizando el repo + este archivo) puedan saber en cualquier momento el estado real del proyecto sin tener que preguntar.
...
2. **Cada fase vive en su propia rama** (`fase-N-nombre-corto`) y no se fusiona a `main` hasta que cumple su "Definición de Terminado". Si algo no se logró completar, se sube igual a esa rama (no se pierde el trabajo) y se marca como `Bloqueada` con una nota de qué falta, para que el otro la retome.
...
| 0 | Setup del entorno | Marcos + Samuel | Terminada y probada (por Marcos — pendiente validar en máquina de Samuel) | `fase-0-setup` |
   | 1 | Esqueleto (Orquestador + interfaz + agente eco) | Marcos | Terminada y probada | `fase-1-esqueleto` |
| 2 | Identity Gateway + Policy Engine básico | Samuel | Terminada y probada (por Marcos) | `fase-2-identidad-llm` |
| 3 | Modelo de permisos + niveles de riesgo + confirmación | Marcos | Terminada y probada | `fase-3-policy-engine` |
| 4 | Agentes reales (Calendar, File, Telegram send) | Marcos | Terminado y probada | `fase-4-agentes-reales` |
...
La asignación de responsables es un punto de partida — se puede intercambiar, lo importante es que quede siempre claro en esta tabla quién la tiene en manos ahora mismo. Y aunque una fase tenga un responsable, **ambos deben entender lo que se construyó**, porque la terna les puede preguntar sobre cualquier parte del sistema.
...
Esto es lo que le permite a cualquiera (ustedes o una IA analizando el repo) mirar `main` y saber que todo lo que está ahí ya fue probado — y mirar las ramas abiertas para saber qué está en progreso o trabado.
...
El orden es lineal a propósito: cada fase depende de que la anterior esté realmente terminada y probada, no a medias. No conviene adelantar trabajo de una fase futura mientras la actual está `Bloqueada` — mejor destrabarla primero, aunque sea con una versión simple, y anotar la mejora pendiente como nota para más adelante.
...
3. Dejar un comentario o issue en GitHub describiendo específicamente qué falla (error, decisión pendiente, dependencia externa) para que el otro pueda retomarlo sin tener que reconstruir el contexto desde cero.
